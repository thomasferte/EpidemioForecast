...
plot_pi_rc_all <- df_pi |> 
  ...
  tidyr::pivot_longer(cols = c(pi_inf, pi_sup)) |>
  ...
  geom_line(mapping = aes(y = pred, color = "Predicted"), linewidth = .7) +
  geom_line(mapping = aes(y = outcome, color = "Observed"), linewidth = .7) +
  scale_color_manual(values = c(viridis::viridis(n = 4, begin = 0.1, end = 0.9), "grey", "black")) +
...
vec_train_test_labeller <- c("Forecast",
                             "Hyperparameters optimization")
names(vec_train_test_labeller) <- c("Test", "Train")
```
...
df_performance_vs_time <- df_emissions %>%
  filter(starting_date == "2021-03-01") %>%
  group_by(rep, model, train_test, starting_date, features) %>%
  summarise(total_mean_time = sum(total_mean_time),
            .groups = "drop") %>%
  group_by(model, train_test, starting_date, features) %>%
  summarise(median_total_mean_time = median(total_mean_time),
            min_total_mean_time = min(total_mean_time),
            max_total_mean_time = max(total_mean_time),
            .groups = "drop")

plot_performance_vs_time <- df_performance_vs_time %>%
  ggplot(mapping = aes(y = model, x = median_total_mean_time,
                       xmin = min_total_mean_time, xmax = max_total_mean_time,
                       color = model,
                       shape = features)) +
  geom_point(size = 3, position = position_dodge(width = .5)) +
  geom_errorbarh(position = position_dodge(width = .5), width = 0) +
  scale_color_manual(values = color_methods) +
  scale_shape_manual(values = c(8, 18)) +
  scale_x_log10() +
  annotation_logticks(sides = "b") +
  facet_wrap(. ~ train_test,
             labeller = labeller(train_test = vec_train_test_labeller),
             ncol = 2) +
  theme_bw() +
  labs(shape = "Features",
       color = "Model",
       y = "Model",
       x = "Computation Time (h)")
...
#| fig-height: 4
#| label: fig-emissionvsperformancelog
#| 
...
ggsave(emissionvsperformance,
       filename = here::here("figures/emissionvsperformancelog.pdf"),
       height = 4,
       width = 5)
```

```{r}
#| fig-cap: "Mean Absolute Error, Carbon footprint and computation time by algorithm starting at 2021-03-01."
#| fig-height: 8
#| label: fig-plot_carbon_time

plot_carbon_time <- ggpubr::ggarrange(emissionvsperformance, plot_performance_vs_time,
                  ncol = 1,
                  common.legend = TRUE, legend = "right",
                  labels = c("A", "B"))

ggsave(plot_carbon_time,
       filename = here::here("figures/plot_carbon_time.pdf"),
       height = 5,
       width = 5.5)
