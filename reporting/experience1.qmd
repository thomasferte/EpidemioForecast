...
plot_pi <- df_pi |> 
  ...
  mutate(pi_method = if_else(name %in% c("outcome", "pred"),
         ...
         pi_method = factor(pi_method,
                            levels = c("outcome", "pred", "Ad hoc", "Conformal", "Residuals"),
                            labels = c("Observed", "Predicted", "Ad hoc", "Conformal", "Residuals"))) |> 
  ...
  scale_color_manual(values = c("black", "grey", viridis::viridis(n = 3, begin = 0.1, end = 0.9))) +
...
plot_coverage <- df_pi_coverage |> 
  ggplot(mapping = aes(x = median_width, y = pct_coverage, color = pi_method, shape = interaction(model, features))) +
  geom_point(size = 2) +
  geom_hline(yintercept = 0.95, linetype = 2) +
  scale_shape_manual(values = c(15, 16, 17, 0, 1, 2)) +
  scale_color_manual(values = c(viridis::viridis(n = 3, begin = 0.1, end = 0.9))) +
  theme_bw() +
