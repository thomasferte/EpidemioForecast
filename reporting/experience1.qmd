...
```{r overallPerf}
#| fig-cap: "Overall performance of the different methods. Erros greater than 50 are set to 50 for visualisation."
#| fig-height: 6
#| 
graph_perf <- ls_prediction_performance$df_prediction_by_day %>%
  fct_compute_performance() %>%
  mutate(AE = if_else(AE > 50, 50, AE)) %>%
...
plot_best_numeric_hp_reservoir <- df_all_hp_best40 |>
  ...
  na.omit() %>%
  mutate(value = if_else(hyperparameter %in% vec_log_hp, log10(value), value),
         hyperparameter = if_else(hyperparameter %in% vec_log_hp,
                                  paste0("log(", hyperparameter, ")"),
                                  hyperparameter)) %>%
...
```{r}
df_emissions %>%
  filter(starting_date == "2020-09-02") %>%
